...
- cli.py uses Hydra's compose API (`initialize_config_dir` on the packaged config/), not `@hydra.main`: hydra-core 1.3.7 (latest stable) crashes in argparse on Python 3.14. `pot_from_config()` turns `height` + `design.*` into a `Pot` (the design keys are exactly the `Pot` field names).
